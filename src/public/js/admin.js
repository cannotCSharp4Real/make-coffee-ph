...
async function loadOrders() {
    try {
        ...
        const response = await fetch(`/api/admin/orders?status=${status}&date=${date}`, {
            headers: {
...
async function generateReport() {
    try {
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(`/api/admin/sales?startDate=${startDate}&endDate=${endDate}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
        ...
        document.getElementById('reportSummary').innerHTML = `
            ...
            <div class="summary-card">
                <h3>Total Orders</h3>
                <div class="value">${report.orderCount}</div>
            </div>
            <div class="summary-card">
                <h3>Average Order Value</h3>
                <div class="value">${formatPrice(report.totalSales / report.orderCount)}</div>
            </div>
        `;
